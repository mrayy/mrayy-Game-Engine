...
cv::name(xMat, yMat, resultMat);\
	...
	template <class S, class D>
	void rotate(S& src, D& dst, double angle, video::SColor fill = video::DefaultColors::Black, int interpolation = INTER_LINEAR) {
		imitate(dst, src);
		Mat srcMat = toCv(src), dstMat = toCv(dst);
	...
	math::matrix4x4 estimateAffine3D(std::vector<math::vector3d>& from, std::vector<math::vector3d>& to, float accuracy = .99);
	math::matrix4x4 estimateAffine3D(std::vector<math::vector3d>& from, std::vector<math::vector3d>& to, vector<unsigned char>& outliers, float accuracy = .99);
}
}