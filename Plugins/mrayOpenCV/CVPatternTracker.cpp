...
namespace video {
    ...
    CVPatternTracker::CVPatternTracker()
    ...
    void CVPatternTracker::setup(){
        m_detector = new cv::OrbFeatureDetector(500); // cv::ORB(1000);
        m_extractor = new cv::OrbDescriptorExtractor(); // cv::FREAK(false, false)
		m_matcher = new cv::BFMatcher(cv::NORM_HAMMING2,true); // cv::BFMatcher(cv::NORM_HAMMING, true)
    }
    
    void CVPatternTracker::add(const cv::Mat& image)
    {
    ...
    bool CVPatternTracker::find(const cv::Mat& image)
    {
        if(rescale == 1) {
            m_img = image;
        } else {
    ...
    void CVPatternTracker::getPose(const cv::Mat &cameraMatrix, const cv::Mat &distCoeffs, cv::Mat &rvec, cv::Mat &tvec){
        solvePnP(m_pattern.points3d, m_info.points2d, cameraMatrix, distCoeffs, rvec, tvec);
    }
    
    
    
    Pattern CVPatternTracker::buildPatternFromImage(const cv::Mat& image) const
    {
        Pattern pattern;
    ...
    void CVPatternTracker::getGray(const cv::Mat& image, cv::Mat& gray)
    {
        if (image.channels()  == 3)
    ...
    bool CVPatternTracker::extractFeatures(const cv::Mat& image, std::vector<cv::KeyPoint>& keypoints, cv::Mat& descriptors) const
    {
        assert(!image.empty());
    ...
    void CVPatternTracker::getMatches(const cv::Mat& queryDescriptors, std::vector<cv::DMatch>& matches)
    {
        matches.clear();
    ...
    bool CVPatternTracker::refineMatchesWithHomography
    (
