...
/*
the CVTracker is used for tracking the identities of a collection of objects that
change slightly over time. example applications are in contour tracking and
face tracking. when using a CVTracker, the two most important things to know are
the persistence and maximumDistance. persistence determines how many frames an
object can last without being seen until the CVTracker forgets about it.
maximumDistance determines how far an object can move until the CVTracker
considers it a new object.

the default CVTrackers are for cv::Rect and cv::Point2f (RectCVTracker and
PointCVTracker). to create a new kind of CVTracker, you need to add a
...
function. in practical terms, the CVTracker can become non-realtime when
tracking more than a few hundred objects. to optimize the CVTracker, consider
avoiding usage of sqrt() in the trackingDistance() function.

this CVTracker doesn't find a global minimum, but a local minimum. for example:
...
tracked/labeled objects. use the CVTrackerFollower extension for this: create
your own MyFollower extending Follower, and create CVTrackerFollower<MyFollower>.

for example:
class MyFollower : public ofxCv::PointFollower { ... }
ofxCv::PointCVTrackerFollower<MyFollower> CVTracker;

then whenever you call CVTracker.track(), the CVTracker will maintain a list of
...
namespace video {
	...
	template <class T>
	void CVTracker<T>::setPersistence(unsigned int persistence) {
		this->persistence = persistence;
	}

	template <class T>
	void CVTracker<T>::setMaximumDistance(float maximumDistance) {
		this->maximumDistance = maximumDistance;
	}

	template <class T>
	const vector<unsigned int>& CVTracker<T>::track(const vector<T>& objects) {
		previous = current;
		int n = objects.size();
		int m = previous.size();
	...
	template <class T>
	const vector<unsigned int>& CVTracker<T>::getCurrentLabels() const {
		return currentLabels;
	}

	template <class T>
	const vector<unsigned int>& CVTracker<T>::getPreviousLabels() const {
		return previousLabels;
	}

	template <class T>
	const vector<unsigned int>& CVTracker<T>::getNewLabels() const {
		return newLabels;
	}

	template <class T>
	const vector<unsigned int>& CVTracker<T>::getDeadLabels() const {
		return deadLabels;
	}

	template <class T>
	unsigned int CVTracker<T>::getLabelFromIndex(unsigned int i) const {
		return currentLabels[i];
	}

	template <class T>
	int CVTracker<T>::getIndexFromLabel(unsigned int label) const {
		return getCurrent(label).getIndex();
	}

	template <class T>
	const T& CVTracker<T>::getPrevious(unsigned int label) const {
		return previousLabelMap.find(label)->second->object;
	}

	template <class T>
	const T& CVTracker<T>::getCurrent(unsigned int label) const {
		return currentLabelMap.find(label)->second->object;
	}

	template <class T>
	bool CVTracker<T>::existsCurrent(unsigned int label) const {
		return currentLabelMap.count(label) > 0;
	}

	template <class T>
	bool CVTracker<T>::existsPrevious(unsigned int label) const {
		return previousLabelMap.count(label) > 0;
	}

	template <class T>
	int CVTracker<T>::getAge(unsigned int label) const{
		return currentLabelMap.find(label)->second->getAge();
	}

	template <class T>
	int CVTracker<T>::getLastSeen(unsigned int label) const{
		return currentLabelMap.find(label)->second->getLastSeen();
	}

	class RectCVTracker : public CVTracker<cv::Rect> {
	protected:
		float smoothingRate;
	...
	public:
		...
		const vector<unsigned int>& track(const vector<cv::Rect>& objects) {
			const vector<unsigned int>& labels = CVTracker<cv::Rect>::track(objects);
			...
			for (int i = 0; i < labels.size(); i++) {
				...
				if (smoothed.count(label) > 0) {
					...
					smooth.x = math::lerp<float>(smooth.x,  cur.x, smoothingRate);
					smooth.y = math::lerp<float>(smooth.y, cur.y, smoothingRate);
					smooth.width = math::lerp<float>(smooth.width, cur.width, smoothingRate);
					smooth.height = math::lerp<float>(smooth.height, cur.height, smoothingRate);
				}
				else {
					smoothed[label] = cur;
				}
	...
	typedef CVTracker<cv::Point2f> PointCVTracker;
	...
	class CVTrackerFollower : public CVTracker<T> {
	protected:
		vector<unsigned int> labels;
		vector<F> followers;
	public:
		const vector<unsigned int>& track(const vector<T>& objects) {
			CVTracker<T>::track(objects);
			// kill missing, update old
			for (int i = 0; i < labels.size(); i++) {
				unsigned int curLabel = labels[i];
				F& curFollower = followers[i];
				if (!CVTracker<T>::existsCurrent(curLabel)) {
					curFollower.kill();
				}
				else {
					curFollower.update(CVTracker<T>::getCurrent(curLabel));
				}
			}
			// add new
			for (int i = 0; i < CVTracker<T>::newLabels.size(); i++) {
				unsigned int curLabel = CVTracker<T>::newLabels[i];
				labels.push_back(curLabel);
				followers.push_back(F());
				followers.back().setup(CVTracker<T>::getCurrent(curLabel));
	...
	template <class F> class RectCVTrackerFollower : public CVTrackerFollower<cv::Rect, F> {};
	template <class F> class PointCVTrackerFollower : public CVTrackerFollower<cv::Point2f, F> {};
}
