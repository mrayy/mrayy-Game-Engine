...
#define OPUS_ENC
	void BuildString()
	{
		...
		core::string audioStr = "appsrc name=src  ";

		audioStr += " ! audio/x-raw,endianness=1234,signed=true,width=16,depth=16,rate=" + core::StringConverter::toString(m_audioGrabber->GetSamplingRate()) + 
			",channels=" + core::StringConverter::toString(m_audioGrabber->GetChannelsCount()) + " "
...
#define SET_SINK(name,p) m_##name=GST_MyUDPSink(gst_bin_get_by_name(GST_BIN(GetPipeline()), #name)); if(m_##name){m_##name->SetPort(m_ipAddr,p);}
	...
	GstFlowReturn NeedBuffer( GstBuffer ** buffer)
	{
		...
		if (!m_audioGrabber)
		{
			gLogManager.log("GstAppNetAudioStreamer::NeedBuffer() -No audio grabber is assigned to GstAppNetAudioStreamer", ELL_WARNING);
	...
	bool CreateStream()
	{
		...
		gLogManager.log("GstAppNetAudioStreamer::Starting with pipeline: " + m_pipeLineString, ELL_INFO);
		if (err)
		{
			gLogManager.log("GstAppNetAudioStreamer: Pipeline error: " + core::string(err->message), ELL_WARNING);
			gst_object_unref(p);
			p = 0;
		}
		if (!p)
			return false;
		gLogManager.log("GstAppNetAudioStreamer::Finished Linking Pipeline", ELL_INFO);
