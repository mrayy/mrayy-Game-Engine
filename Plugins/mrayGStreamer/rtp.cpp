...
unsigned char rtp_padding_payload(unsigned char *src, int len, unsigned char *data){
	rtp_header *rtp = (rtp_header*)src;
	if (rtp->timestamp == 0 || rtp->padding==0) return -1;
...
unsigned short rtp_add_padding(unsigned char *src, int len, unsigned char *data, int datalen, unsigned char * newdata)
{
	rtp_header *rtp = (rtp_header*)src;
	...
	if (rtp->padding!=0)
		padding_len=src[len - 1];
	else rtp->padding = 1;
	memcpy(newdata, src, len);
	memcpy(newdata + len - 1, data, datalen);
	len = len + datalen;
	newdata[len - 1] = padding_len + datalen;
