...
#define SET_SINK(name,p) m_##name=GST_MyUDPSink(gst_bin_get_by_name(GST_BIN(GetPipeline()), #name)); if(m_##name){m_##name->SetPort(m_ipAddr,p);}
	...
	void BindPorts(const std::string& addr, uint videoPort, bool rtcp)
	{
		m_ipAddr = addr;
...
void GstNetworkVideoStreamer::BindPorts(const std::string& addr, uint* videoPort, uint count, bool rtcp)
{
