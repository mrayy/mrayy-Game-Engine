...
namespace mray
{
	namespace video
	{
		...
		void ICustomVideoSrc::SetEncoderType(const std::string &type)
		{
			...
			{
				m_encoderParams["end-usage"] = "cq";
				m_encoderParams["deadline"] = "1";
				m_encoderParams["cpu-used"] = "5";
				m_encoderParams["threads"] = "2";
			}
		}
		...
		std::string ICustomVideoSrc::BuildStringVP9(int i)
		{
			std::string videoStr = "";

			videoStr += "  ! videoconvert ! vp9enc  target-bitrate=" + core::StringConverter::toString((1000 * m_bitRate) / GetStreamsCount()) + " ";
			...
			videoStr += " ! rtpvp9pay mtu=" + core::StringConverter::toString(m_mtuSize) + " ";
			/*
			//videoStr += " ! vp8enc ! rtpvp8pay ";
			videoStr += " ! theoraenc ! rtptheorapay ";*/
