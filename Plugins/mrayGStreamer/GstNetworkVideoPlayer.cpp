...
namespace video
{
	...
	public:
		...
		void Start()
		{
			m_lastTime = gGStreamerCore->GetTimer()->getSeconds();
...
// 		SET_SRC(videoRtcpSrc, (m_videoPort + 2));
	...
	bool CreateStream()
	{
		...
		if (err)
		{
			gLogManager.log("GstNetworkVideoPlayer: Pipeline error: "+ core::string(err->message),ELL_WARNING);
		}
		if (!p)
			return false;
		SetPipeline(p);
