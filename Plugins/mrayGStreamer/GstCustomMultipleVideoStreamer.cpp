		...
		for (int i = 0; i < m_grabber.size(); ++i)
		{
			if (m_grabber[i])
			{
				...
				videoStr += "! x264enc bitrate=" + core::StringConverter::toString(m_bitRate / m_grabber.size()) +
						" speed-preset=superfast pass=qual tune=zerolatency sync-lookahead=0 rc-lookahead=0 sliced-threads=true "// ip-factor=1.8 interlaced=true sliced-threads=false  "// 
					" ! rtph264pay ";
					/*
