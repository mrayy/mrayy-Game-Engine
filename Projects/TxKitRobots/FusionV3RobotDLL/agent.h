...
{
public:
	char headCOM[10], gyroheadCOM[10], robotCOM[10], LArmCOM[10], RArmCOM[10];
	...
	if (!confFile.is_open()) {
		...
		while (!confFile.eof()) {
			...
			if (token[0]) {
				...
				if (strcmp(token[0], "LARM_COM_PORT") == 0)
					strcpy(_config.LArmCOM, token[1]);

				if (strcmp(token[0], "RARM_COM_PORT") == 0)
					strcpy(_config.RArmCOM, token[1]);
