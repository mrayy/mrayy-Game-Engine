...
CMemoryStream::CMemoryStream(const  std::string&name, byte*data, int size, bool deleteAtEnd, FILE_MODE _mode) : IStream(name, _mode), bDeleteAtEnd(deleteAtEnd){
...
bool CMemoryStream::open(const  std::string&name, byte*data, int size, FILE_MODE _mode){
	m_streamName=name;
	m_mode=_mode;
	setData(data,size);
	return 1;
