...
public:
	CMemoryStream();
	CMemoryStream(const  std::string&name,byte*data,int size,bool deleteAtEnd,FILE_MODE _mode=BIN_READ);
	...
	bool open(const  std::string&name, byte*data, int size, FILE_MODE _mode);
	virtual void close();
	virtual bool isOpen();
