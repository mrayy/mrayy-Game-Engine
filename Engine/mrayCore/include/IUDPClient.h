...
namespace network
{
	...
	// The mT("Win32UDPClientOwner" class  exists only for the derivation of 
...
public:
	...
	static core::string TranslateErrorMessage(UDPClientError e)
	{
		switch(e)
		{
		case UDP_SOCKET_ERROR_NONE:		return mT("None");
		case UDP_SOCKET_ERROR:			return mT("Generic Error");
		case UDP_SOCKET_ERROR_OPEN:		return mT("Open Socket");
		case UDP_SOCKET_ERROR_BIND:		return mT("Bind Socket");
		case UDP_SOCKET_ERROR_CONNECT:	return mT("Connect");
		case UDP_SOCKET_ERROR_NAME:		return mT("getsockname()");
		case UDP_SOCKET_ERROR_SEND:		return mT("Send");
		case UDP_SOCKET_ERROR_RECV:		return mT("Receive");
		case UDP_SOCKET_ERROR_MCAST:	return mT("Join group error");
		case UDP_SOCKET_ERROR_TTL:		return mT("MCAST Time To Live (TTL)");
		case UDP_SOCKET_ERROR_IFACE:	return mT("MCAST iFace");
		case UDP_SOCKET_ERROR_TOS:		return mT("ip tos");
		case UDP_SOCKET_ERROR_LOOP:		return mT("loopback");
		case UDP_SOCKET_ERROR_REUSE:	return mT("port/addr reuse");
		case UDP_SOCKET_ERROR_SEC:		return mT("ipsec");
		}
		return mT("");
	}
};
