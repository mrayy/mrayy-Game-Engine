...
#ifndef _IRR_DEBUG_BREAK_IF
#define _IRR_DEBUG_BREAK_IF(c)if(c) {_asm int 3}
#endif
//!	Self reallocating template array (like stl vector) with additional features.
/** Some features are: Heap sorting, binary search methods, easier debugging.
*/
template <class  T, typename TAlloc = HeapAllocator<T> >
class  array
{
...
//			allocator.deallocate(data);
	...
	void reallocate(uint new_size,bool copy=true)
	{/*
		try{*/
			T* old_data =0;
			if(data)
		...
		//	memset(data,0,new_size*sizeof(T));
			...
			if(old_data){
				if(copy){
					...
					for (; lastConstructed<end; ++lastConstructed)
					{
						//data[i] = old_data[i];
						allocator.construct(&data[lastConstructed], old_data[lastConstructed]);
						//IOSSystem::memCopy(data,old_data,end*sizeof(T));
					}
				}
			...
			*/
			...
			for (; lastConstructed<new_size; ++lastConstructed)
			{
				//data[i] = old_data[i];
				allocator.construct(&data[lastConstructed]);
				//IOSSystem::memCopy(data,old_data,end*sizeof(T));
			}
			/*
		}
		catch(...){
		//	FATAL_ERROR(true,"Array::reallocate()","exception while Allocating");

		}*/
	}

	//! Reallocates the array, make it bigger by the size.
	//! \param new_size: additional size for the array.
	void increaseUsed(int additional_size,bool copy=true){
		resize(size()+additional_size,copy);
	}
	...
	void insert(const T &element, uint index=0)
	{
	...
	void set_pointer(T* newPointer, uint size)
	{
	/*	for (uint i=0; i<used; ++i)
			allocator.destruct(&data[i]);
*/
		allocator.deallocate(data);//delete [] data;// // delete [] data;
		data = newPointer;
		allocated = size;
		used = size;
		is_sorted = false;
	...
	void resize(uint usedNow,bool copy=true)
	{
		if (allocated < usedNow)
			reallocate(usedNow,copy);
...
*/
	...
	void erase(uint index, int count)
	{
