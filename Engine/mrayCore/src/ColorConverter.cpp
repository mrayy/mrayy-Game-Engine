		...
		int linePad)
{
	...
	{
		clrOut-=size.x;
		IOSSystem::memCopy(clrOut,clrIn,size.x*sizeof(uchar));
		clrIn++;
		clrIn+=linePad;
	}
}
	...
	int linePad,bool flip,bool bgr)
{
	...
	for(int y=0;y<size.y;y++)
	{
		...
		{
			IOSSystem::memCopy(clrOut,clrIn,lineWidth*sizeof(uchar));
		}
...
void ColorConverter::createNormalMap(video::ImageInfo*img,float amplitude){
	...
	IOSSystem::memCopy(in,ptr,sizeof(uchar)*d.y*pitch);
