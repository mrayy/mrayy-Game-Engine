...
CPUInfo::CPUInfo ()
{
	...
	if (DoesCPUSupportCPUID ()) {
		...
		gLogManager.startSection(mT("CPU"));

		core::string s;

		s=mT("Processor Vendor: ");
		s+=core::StringConverter::toString(ChipID.Vendor);
		gLogManager.log(s,ELL_INFO);

		s=mT("Processor Name: ");
		s+=core::StringConverter::toString(ChipID.ProcessorName);
		gLogManager.log(s,ELL_INFO);

		s=mT("Speed: ");
		int spd=Speed->CPUSpeedInMHz;
		s+=core::StringConverter::toString(spd);
		s+=mT(" mhz");
		gLogManager.log(s,ELL_INFO);
...
bool __cdecl CPUInfo::RetrieveCPUClockSpeed ()
{
	...
	// Get the clock speed.
	Speed = new CPUSpeed ();
	if (Speed.pointer() == NULL) return false;
...
CPUInfo::CPUSpeed::CPUSpeed ()
{
	...
	for (unsigned int nCounter = 0; nCounter < uiRepetitions; nCounter ++) {
		i64Total += GetCyclesDifference (CPUInfo::CPUSpeed::Delay, uiMSecPerRepetition);
		i64Overhead += GetCyclesDifference (CPUInfo::CPUSpeed::DelayOverhead, uiMSecPerRepetition);
	}
...
__int64	__cdecl CPUInfo::CPUSpeed::GetCyclesDifference (DELAY_FUNC DelayFunction, unsigned int uiParameter)
{
...
void CPUInfo::CPUSpeed::Delay (unsigned int uiMS)
{
...
void CPUInfo::CPUSpeed::DelayOverhead (unsigned int uiMS)
{
	LARGE_INTEGER Frequency, StartCounter, EndCounter;
	__int64 x;
